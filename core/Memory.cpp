...
uint16 Memory::read_word(uint18 address, uint18 base_address) {
  ...
  if ((address & 0x1) == 0x1)
    throw new runtime_error("Illegal memory address access");
  uint16 val = _memory_array[address - base_address + 1] | (_memory_array[address - base_address] << 8);
  return val;
}
...
void Memory::write_word(uint18 address, uint18 base_address, uint16 value) {
  ...
  if ((address & 0x1) == 0x1)
    throw new runtime_error("Illegal memory address access");
  _memory_array[address - base_address + 1] = (uint8) (value & 0xFF);
  _memory_array[address - base_address] = (uint8) ((value & 0xFF00) >> 8);
}
